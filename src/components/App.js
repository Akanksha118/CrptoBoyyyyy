...
class App extends Component {
  ...
  setMetaData = async () => {
    if (this.state.cryptoBoys.length !== 0) {
      this.state.cryptoBoys.map(async (cryptoboy) => {
        const client = new Web3Storage({ token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDM1NDdDZjcxM0YxNTg2NjA5ZUY3ZDNGNDRFODhlNTUwMzNBMTkwMTIiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2Njg4NDc2MTMwOTAsIm5hbWUiOiJDcnlwdG9Cb3kifQ.TLKRg8b_M61e_UFCNNxonVB8yf0Yty2v0zMFeeXdmN4" });
        const result = await client.get(cryptoboy.tokenURI);
  ...
  mintMyNFT = async (colors, name, tokenPrice) => {
    ...
    if (colorsUsed.length === 0 && !nameIsUsed) {
      ...
      const blob = new Blob([JSON.stringify(tokenObject)], { type: 'application/json' });
      console.log(blob);
